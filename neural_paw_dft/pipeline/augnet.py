...
from neural_paw_dft.augnet.augnet_model import Z_TO_SCHEMA, lmaxmix_component_mask
...
from neural_paw_dft.models import augnet_sidecar, resolve_weights


class AugNetPredictor:
    """One loaded checkpoint (total or mag channel) plus its target transform."""

    def __init__(self, ckpt_path: Path, stats_path: Path, device: torch.device,
        ...
        if ckpt_path.suffix == ".safetensors":
            ...
            if enable_cueq:
                raise ValueError(f"{ckpt_path.name}: published safetensors are in e3nn layout; enable_cueq is not supported")
            conf = json.loads(augnet_sidecar(ckpt_path).read_text())
            self.channel = conf["target_channel"]
            self.r_max = float(conf["r_max"])
            self.model = PAWAugModel(
                hidden_irreps=hidden_irreps_from_width(conf["hidden_width"], conf["max_ell"]),
                atomic_numbers=atomic_numbers,
                target_channel=self.channel,
                r_max=self.r_max,
                max_ell=int(conf["max_ell"]),
                num_interactions=int(conf["num_interactions"]),
                correlation=int(conf["correlation"]),
                avg_num_neighbors=float(conf["avg_num_neighbors"]),
                head_rank=int(conf["head_rank"]),
                head_block_mixing=bool(conf["head_block_mixing"]),
                head_linear_max_l=int(conf["head_linear_max_l"]),
                use_reduced_cg=bool(conf["use_reduced_cg"]),
                cueq_config=None,
                oeq_config=None,
            )
            ...
            self.channel = hp["target_channel"]
...
def load_augnet(checkpoint: str, stats: str | None, device: torch.device, enable_cueq: bool | None = None,
