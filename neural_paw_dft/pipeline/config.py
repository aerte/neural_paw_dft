...
class ElectrafiConfig:
    checkpoint: str = "electrafi_spin_constrained"  # registry name (neural_paw_dft.models) or path
    train_config: str | None = None  # training yaml; required unless checkpoint is a registry name
...
class AugnetConfig:
    ...
    enable_cueq: bool | None = None  # only for .ckpt files (None -> stored value); published safetensors reject True
...
class PipelineConfig:
    ...
    grid_dims: tuple[int, int, int] | None = None  # None -> CHGCAR input -> NGXF/NGYF/NGZF in incar_overrides -> dry run -> error
    out_dir: str = "ndi_out"
    weights_dir: str | None = None  # None -> $NDI_WEIGHTS_DIR, <repo>/trained_models (editable) or ~/.cache/neural_paw_dft
...
def config_template() -> str:
    return """\
# neural_paw_dft pipeline configuration (all keys optional; shown with their defaults)
device: auto                 # auto | cpu | cuda | cuda:0
matmul_precision: high       # torch float32 matmul precision; "high" (TF32 on Ampere+) matches training
grid_dims: null              # [NGX, NGY, NGZ]; null -> CHGCAR input, else NGXF/NGYF/NGZF in incar_overrides, else VASP dry run, else error
out_dir: ndi_out
weights_dir: null            # null -> $NDI_WEIGHTS_DIR, <repo>/trained_models (editable install) or ~/.cache/neural_paw_dft

electrafi:
  checkpoint: electrafi_spin_constrained   # registry name, or a path with train_config set
  train_config: null         # null -> the yaml registered for the checkpoint name
...
augnet:
  ...
  enable_cueq: null          # only for .ckpt files; the published safetensors are e3nn layout (true raises)
