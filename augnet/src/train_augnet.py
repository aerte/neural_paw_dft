...
from mace.modules.wrapper_ops import CuEquivarianceConfig, OEQConfig, get_layout

from src.run_paw_chgcar import (
...
from src.augnet_model import Z_TO_SCHEMA, lmaxmix_component_mask
from src.paw_head_shared import SharedPAWHead
from src.paw_moments import moment_regularization_loss, moments_from_coeffs
from src.paw_stats import PAWStats, compute_paw_stats, standardize, de_standardize
...
def resolve_avg_num_neighbors(cfg: SimpleNamespace) -> float:
    ...
    if not Path(path).exists():
        raise FileNotFoundError(
            f"model.avg_num_neighbors: auto but {path} does not exist. "
            "Run scripts/neighbor_stats.py for this split."
        )
...
class LightningPAWModule(pl.LightningModule):
    def __init__(
        self,
        model: nn.Module,
        cfg: SimpleNamespace,
...
def main():
    ...
    if use_stats_file:
        if not Path(stats_file).exists():
            raise FileNotFoundError(f"stats_file not found: {stats_file}")
