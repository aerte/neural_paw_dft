...
from src.paw_basis_transform import set_l_channel_overrides, sanvito_to_e3nn_with_basis_padded
from src.augnet_model import Z_TO_SCHEMA, SCHEMA_L_CHANNELS, build_sanvito_blocks
from src.paw_moments import reconstruct_D
...
def physical_l_channels(z: int) -> list[int]:
    ...
    from src.paw_basis_transform import _L_CHANNEL_OVERRIDES
...
def _atomic_numbers(chgcar_path: str) -> np.ndarray:
    from src.run_paw_chgcar import read_text_maybe_lz4, _atomic_numbers_from_chgcar_header
...
def read_truth(chgcar_path: str, atomic_numbers: np.ndarray) -> np.ndarray:
    ...
    from src.run_paw_chgcar import parse_aug_from_file, pack_aug_padded_sanvito
