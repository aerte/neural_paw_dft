#!/usr/bin/env python3
"""Submit all slices of the sad_grid_conv_aug experiment (Experiment B).

Per-row VASP seed = SAD-CHGCAR grid + converged-CHGCAR augmentation.
Same shape as submit_sad_diff_hybrid.py (passes --sad_cache), but for the
sad_grid_conv_aug experiment only.

Usage (from the project root on the cluster):
    python submissions/submit_sad_grid_conv_aug.py
    python submissions/submit_sad_grid_conv_aug.py --dry-run
    python submissions/submit_sad_grid_conv_aug.py --num-slices 20
    python submissions/submit_sad_grid_conv_aug.py --partition <name>
"""
import argparse
import re
import os
...
EXPERIMENTS = [
    {
        "tag": "sad_grid_conv_aug",
        "src_csv": "exps/grid_sad_aug_conv.csv",
        "runner": "run_sad_grid_conv_aug_mp.py",
        "results_prefix": "MP_SAD_GRID_CONV_AUG",
        "outdir_prefix": "SAD_GRID_CONV_AUG_RUNS_MP",
    },
]
