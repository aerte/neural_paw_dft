#!/usr/bin/env python3
"""Submit all slices of the conv_grid_sad_aug experiment (Experiment A).

Per-row VASP seed = converged-CHGCAR grid + SAD-CHGCAR augmentation.
Same shape as submit_sad_diff_hybrid.py (passes --sad_cache), but for the
conv_grid_sad_aug experiment only.

Usage (from the project root on the cluster):
    python submissions/submit_conv_grid_sad_aug.py
    python submissions/submit_conv_grid_sad_aug.py --dry-run
    python submissions/submit_conv_grid_sad_aug.py --num-slices 20
    python submissions/submit_conv_grid_sad_aug.py --partition <name>
"""
import argparse
import re
import os
...
EXPERIMENTS = [
    {
        "tag": "conv_grid_sad_aug",
        "src_csv": "exps/grid_conv_aug_sad.csv",
        "runner": "run_conv_grid_sad_aug_mp.py",
        "results_prefix": "MP_CONV_GRID_SAD_AUG",
        "outdir_prefix": "CONV_GRID_SAD_AUG_RUNS_MP",
    },
]
