#!/usr/bin/env python3
"""Submit all slices of the sad_total_hybrid experiment.

Hybrid CHGCAR = SAD total + converged diff (built per-row by the runner).
...
Usage (from the project root on the cluster):
    python submissions/submit_sad_total_hybrid.py
    python submissions/submit_sad_total_hybrid.py --dry-run
    python submissions/submit_sad_total_hybrid.py --num-slices 20
    python submissions/submit_sad_total_hybrid.py --partition <name>
"""
import argparse
import re
import os
...
EXPERIMENTS = [
    {
        "tag": "sad_total_hybrid",
        "src_csv": "exps/default_tasks.csv",
        "runner": "run_sad_total_hybrid_mp.py",
        "results_prefix": "MP_SAD_TOTAL_HYBRID_SCF",
        "outdir_prefix": "SAD_TOTAL_HYBRID_SCF_RUNS_MP",
    },
]
