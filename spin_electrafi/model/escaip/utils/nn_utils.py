...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        norm_x = x.norm(dim=-1, keepdim=True) * (1.0 / (x.size(-1) ** 0.5))
        return self.weight * x / (norm_x + self.eps)

# patch torch.nn if missing
...
def get_normalization_layer(normalization_type: NormalizationType, is_graph=True):
    return {
        ...
        NormalizationType.Skip: Skip if not is_graph else SkipGraph,
