...
#SBATCH --job-name=full_spin_norenorm
#SBATCH -N 1-1
#SBATCH --ntasks-per-node=1
#SBATCH --cpus-per-task=24
#SBATCH --time=50:00:00
#SBATCH --gres=gpu:1
#SBATCH --output=full_spin_norenorm.log
#SBATCH --error=full_spin_norenorm.err
#SBATCH --mem=240G
...
sed -e 's/^data_split:.*/data_split: mpfull2025/' \
    ...
    hpc_conf.yaml > runs/configs/hpc_full_spin_norenorm.yaml
cat >> runs/configs/hpc_full_spin_norenorm.yaml <<'EOF'
...
# The arm-defining change: no net-moment constraint on the spin channel.
spin_renorm: False

model_name: mpfull2025_spin_norenorm
run_name: full_spin_norenorm
EOF

srun python train.py --config runs/configs/hpc_full_spin_norenorm.yaml
