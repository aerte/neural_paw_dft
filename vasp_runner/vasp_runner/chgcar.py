...
def _rewrite_aug_headers(path: str) -> int:
    ...
    Verified by controlled experiment (``exps/seed_ingest_test.py``): rewriting
...
def build_aug_swap_chgcar(grid_src: str, aug_src: str, out_path: str,
    ...
    Used by run_conv_grid_sad_aug_mp.py and run_sad_grid_conv_aug_mp.py
    ...
    (run_conv_grid_sad_aug_mp.py --ispin1). Neither source then needs a 'diff' channel.
...
def build_total_pseudo_grid_chgcar(grid_src: str, aug_src: str, out_path: str,
    ...
    Used by the total-pseudo-grid runner (not included) to seed VASP with an ML-predicted total
