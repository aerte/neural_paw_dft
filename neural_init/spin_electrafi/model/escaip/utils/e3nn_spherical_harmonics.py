"""Real spherical harmonics up to l = 12, vendored verbatim from e3nn 0.5.1
(e3nn/o3/_spherical_harmonics.py::_spherical_harmonics). MIT License, Copyright (c) 2020
The Regents of the University of California and e3nn contributors -- see LICENSE.e3nn in
the escaip package directory.

Why: the trained EScAIP backbones use node_direction_expansion_size = 13, i.e. lmax = 12.
e3nn 0.4.4 (pinned by mace-torch) only implements l <= 11 and silently returns 144 instead
of 169 components for lmax = 12; e3nn >= 0.5 (which fairchem-core pulled in at training
time) has the l = 12 polynomials. Vendoring the 0.5.1 function reproduces the training-time
computation exactly under either e3nn version.
"""
import math
