...
from .e3nn_spherical_harmonics import _spherical_harmonics  # l <= 12 (e3nn 0.4.4 stops at 11)
...
def get_node_direction_expansion(
    ...
    node_boo = compilable_scatter(edge_sh, edge_index[1], dim_size=num_nodes, dim=0, reduce="mean")
    sh_index = torch.arange(lmax + 1, device=node_boo.device)
    sh_index = torch.repeat_interleave(sh_index, 2 * sh_index + 1)
    node_boo = compilable_scatter(node_boo**2, sh_index, dim_size=lmax + 1, dim=1, reduce="sum").sqrt()
    return node_boo
...
def pad_batch(
    ...
    edge_distance = F.pad(edge_distance, (0, 0, 0, pad_size), value=0.0)  # (N+pad, M)
...
def patch_singleton_atom(edge_direction, neighbor_list, neighbor_mask, node_padding_mask=None):
    """
    Patch singleton atoms (with 0 neighbors) by adding a self-loop.
    Padded nodes are ignored.
    """
    # Atoms with no neighbors
    idx = torch.where(neighbor_mask.sum(dim=-1) == 0)[0]

    if node_padding_mask is not None:
        # filter out padded nodes
        idx = idx[node_padding_mask[idx]]
