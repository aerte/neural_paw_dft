"""Periodic radius-graph construction, vendored from fairchem-core 1.10.0.

Source: src/fairchem/core/common/utils.py (get_pbc_distances, radius_graph_pbc,
sum_partitions, get_counts, get_max_neighbors_mask, compute_neighbors) and
src/fairchem/core/models/base.py (GraphData, GraphModelMixin.generate_graph).
MIT License, Copyright (c) Meta Platforms, Inc. and affiliates -- see LICENSE.fairchem
in the escaip package directory.

Modifications:
  * generate_graph is a module-level function (no mixin); the ``x or self.x`` fallbacks
    are dropped because EScAIPBackbone always passes every argument explicitly.
  * The non-periodic branch (torch_geometric.nn.radius_graph, which needs torch_cluster)
    raises NotImplementedError; ELECTRAFI always runs with use_pbc=True.
Everything else is verbatim, so graphs are identical to the fairchem implementation.
"""
...
def generate_graph(
    ...
    """fairchem GraphModelMixin.generate_graph as a free function (see module docstring)."""
    ...
    if use_pbc:
        ...
        raise NotImplementedError(
            "non-periodic graphs need torch_cluster.radius_graph; ELECTRAFI always uses use_pbc=True"
        )
