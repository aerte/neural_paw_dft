...
def _add_common(p):
    ...
    p.add_argument("--no-spin", action="store_true", help="charge-only seed (no magnetization grid)")
...
def main(argv=None) -> int:
    ...
    b = sub.add_parser("build", help="predict and write CHGCAR + INCAR/POSCAR/POTCAR/KPOINTS")
    _add_common(b)
    b.add_argument("--incar", nargs="*", metavar="KEY=VAL", help="INCAR overrides, applied last")

    p = sub.add_parser("predict", help="predict only; write grids (.npy) and augmentation (.npz)")
    _add_common(p)
