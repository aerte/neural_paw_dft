...
from neural_init.models import ELECTRAFI_TRAIN_CONFIG, resolve_weights
...
def _train_config_path(cfg: ElectrafiConfig, ckpt: Path) -> Path:
    ...
    if ckpt.suffix == ".ckpt":  # spin models are Lightning checkpoints
        arm = "unconstrained" if "unconstrained" in ckpt.name else "constrained"
        return resource_path("spin_electrafi", f"configs/{arm}_full_spin.yaml")
    return resource_path("spin_electrafi", "hpc_conf.yaml")  # charge-only flat state dicts
