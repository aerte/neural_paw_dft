...
def resource_path(subpkg: str, rel: str) -> Path:
    """Absolute path of ``rel`` inside ``neural_init.<subpkg>``."""
    return Path(str(files(f"neural_init.{subpkg}").joinpath(rel)))
...
def resolve_data_path(value, subpkg: str) -> Path:
    ...
    same relative path is looked up inside ``neural_init.<subpkg>``, so shipped
    ...
    if cand.is_file():
        return cand
    raise FileNotFoundError(f"{value!r} not found in CWD or inside neural_init.{subpkg}")
